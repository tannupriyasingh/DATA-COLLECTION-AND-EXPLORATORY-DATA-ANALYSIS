library(ggplot2)
graph2 <- read.csv(file.choose())
#function that would appened 0 in front of week number if it is less than 10 
myfunc = function (x) {
  if (x < 10) {
...
graph2$WEEK = sapply(graph2$WEEK, myfunc)
...
type <-
  c(
    rep("1A..Subtyping.not.Performed.", 53),
    rep("2A..2009.H1N1.", 53),
    rep("3A..H3.", 53),
    rep("4H3N2v", 53),
    rep("5B", 53),
